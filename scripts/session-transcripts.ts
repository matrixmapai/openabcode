 ...
 *   --analyze      Spawn openabcode subagents to analyze each transcript file
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".openabcode/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn openabcode subagents for pattern analysis.");
		return;
	}

	// Find AGENTS.md files to compare against
	const globalAgentsMd = join(homedir(), ".openabcode/agent/AGENTS.md");
