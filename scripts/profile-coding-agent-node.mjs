...
const agentDirEnvName = "OPENABCODE_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "OPENABCODE_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific OPENABCODE_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force OPENABCODE_OFFLINE=1 / OPENABCODE_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.OPENABCODE_OFFLINE = "1";
		env.OPENABCODE_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
