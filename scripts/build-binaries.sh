#!/usr/bin/env bash
#
# Build openabcode binaries for all platforms locally.
...
#     openabcode-darwin-arm64.tar.gz
#     openabcode-darwin-x64.tar.gz
#     openabcode-linux-x64.tar.gz
#     openabcode-linux-arm64.tar.gz
#     openabcode-windows-x64.zip
#     openabcode-windows-arm64.zip
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        # Windows (zip)
        echo "Creating openabcode-$platform.zip..."
        (cd "$platform" && zip -r ../openabcode-$platform.zip .)
    else
        # Unix platforms (tar.gz) - use wrapper directory for mise compatibility
        echo "Creating openabcode-$platform.tar.gz..."
        mv "$platform" openabcode && tar -czf openabcode-$platform.tar.gz openabcode && mv openabcode "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    rm -rf "$platform"
    if [[ "$platform" == windows-* ]]; then
        mkdir -p "$platform" && (cd "$platform" && unzip -q ../openabcode-$platform.zip)
    else
        tar -xzf openabcode-$platform.tar.gz && mv openabcode "$platform"
    fi
done
...
for platform in "${PLATFORMS[@]}"; do
    if [[ "$platform" == windows-* ]]; then
        echo "  $OUTPUT_DIR/$platform/openabcode.exe"
    else
        echo "  $OUTPUT_DIR/$platform/openabcode"
    fi
done
