...
import { AuthStorage, type OAuthCredential } from "@openabcode/coding-agent";
...
const DEFAULT_RADIUS_URL = "https://radius.openabcode.com/";
...
export function getRadiusUrl(): string {
	return process.env.OPENABCODE_RADIUS_URL || DEFAULT_RADIUS_URL;
}

export function getRadiusOrchestratorBaseUrl(): string {
	const explicitUrl = process.env.OPENABCODE_RADIUS_ORCHESTRATOR_URL;
	if (explicitUrl) {
		return explicitUrl;
	}
...
export function getRadiusAccessToken(): string {
	...
	const apiKey = process.env.OPENABCODE_RADIUS_API_KEY;
	if (apiKey) {
		return apiKey;
	}

	throw new Error("Radius credentials are required in ~/.openabcode/agent/auth.json or OPENABCODE_RADIUS_API_KEY");
}

export function isRadiusEnabled(): boolean {
	return !!getStoredRadiusCredential()?.access || !!process.env.OPENABCODE_RADIUS_API_KEY;
}
