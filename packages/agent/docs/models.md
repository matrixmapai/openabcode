...
This document describes the target design for the next `@openabcode/ai` model/provider refactor. It describes the desired shape, not the current implementation. It is intended to be complete enough to start implementing from a fresh session.
...
Non-goals for the immediate `@openabcode/ai` pass:
...
The app injects storage; `@openabcode/ai` ships an in-memory default. Keyed by provider id, one credential per provider:
...
`@openabcode/ai/compat` preserves the old global API surface until the coding-agent migration deletes it. New code never imports it.
...
coding-agent (and the interim agent package) switch imports of these symbols from `@openabcode/ai` to `@openabcode/ai/compat` (import-path-only change) and are otherwise untouched until the ModelManager migration.

Extension grace period: the coding-agent extension loader (jiti aliases + Bun `virtualModules`) resolves the `@openabcode/ai` ROOT specifier to the compat entrypoint. Existing user extensions using the old global API (`complete`, `getModel`, `registerApiProvider`, ...) keep working at runtime without changes; they break only when compat is removed at the ModelManager migration, with a migration guide in the changelog. Typechecking is the nudge: editors resolve the root to the slim core types, so extension sources that typecheck must import old globals from `/compat` — which is what the repo example extensions demonstrate.
...
1. Main `@openabcode/ai` import is core-only.
...
- coding-agent still uses legacy `AuthStorage` + `ModelRegistry` and imports old global @openabcode/ai APIs through `@openabcode/ai/compat`.
- The extension loader still aliases the @openabcode/ai root to `/compat` as the runtime grace period for old extensions.
...
- [x] Switch old-global imports to `@openabcode/ai/compat` (landed with Phase 5; compat is a superset so the switch was path-only). Extension loader resolves the @openabcode/ai root to compat as the runtime grace period.
...
- [x] `packages/coding-agent/CHANGELOG.md`: `### Changed` entry for extension authors — runtime unaffected (loader resolves the @openabcode/ai root to compat), typecheck nudges to `/compat` or the new API; removal happens later with a migration guide.
...
coding-agent replaces AuthStorage and ModelRegistry's internals with `FileCredentialStore` + a `MutableModels` collection. AgentSession itself stays (AgentHarness adoption is openabcode 2.0); only its model/auth substrate swaps. Layering is strictly one-directional:
...
1. [x] @openabcode/ai rework first: `Provider.getModels()` sync + optional `refreshModels()`; `Models.getModels`/`getModel` sync, `Models.refresh(provider?)` async; `createProvider` takes `models` array + optional `refreshModels` fetcher (in-flight dedupe). Reverses Phase 1's async-listing decision — see "Provider model listing" for rationale (sync-or-async unions breed latent sync assumptions; async-only breaks sync consumer surfaces like extension `find`/`getAll`).
2. [x] Cloudflare provider auth in @openabcode/ai factories: Workers AI and AI Gateway validate their required account/gateway env/config and return resolved `baseUrl`, provider-scoped env, and header suppression/override metadata from provider auth.
3. [ ] Add `FileCredentialStore` in coding-agent.
   - Implement the @openabcode/ai `CredentialStore` interface as a self-contained `auth.json` store; do not depend on the old `AuthStorageBackend` abstraction, though its lock/retry semantics may be ported.
...
7. [ ] Extension OAuth adapter.
   - Adapt old extension `OAuthProviderInterface` configs to @openabcode/ai `OAuthAuth`.
...
### Phase 10 — compat deletion (openabcode 2.0 era, separate)
...
- [ ] Delete `/compat`, `env-api-keys.ts`, the extension-loader root-to-compat alias, the old `@openabcode/ai/oauth` registry and `OAuthProviderInterface` (incl. `usesCallbackServer`), and the compat-local legacy API registry. This is the extension-author breaking release; changelog carries the migration guide.
