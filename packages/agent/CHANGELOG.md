...
- Removed the `@openabcode/agent-core/base` selective-provider entrypoint; use the root package with an explicit `Models` instance instead.
...
- Added `@openabcode/agent-core/base` for bundlers that want to pair the agent core with selective `@openabcode/ai/base` provider registration ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
