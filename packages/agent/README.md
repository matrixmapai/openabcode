...
Stateful agent with tool execution and event streaming. Built on `@openabcode/ai`.

## Installation

```bash
npm install @openabcode/agent-core
```
...
```typescript
declare module "@openabcode/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@openabcode/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@openabcode/agent-core";
