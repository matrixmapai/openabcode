...
Extensions can register custom model providers via `openabcode.registerProvider()`. This enables:
...
export default function (openabcode: ExtensionAPI) {
  ...
  openabcode.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. openabcode waits for the factory before startup continues, so the provider is available during interactive startup and to `openabcode --list-models`.
...
openabcode.registerProvider("openai", {
  headers: {
...
openabcode.registerProvider("google", {
...
```typescript
openabcode.registerProvider("my-llm", {
...
Use `openabcode.unregisterProvider(name)` to remove a provider that was previously registered via `openabcode.registerProvider(name, ...)`:
...
// Later, remove it
openabcode.unregisterProvider("my-llm");
```
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map openabcode levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
openabcode.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@openabcode/ai";
...
Credentials are persisted in `~/.openabcode/agent/auth.json`:
...
- [anthropic.ts](https://github.com/matrixmapai/openabcode/blob/main/packages/ai/src/providers/anthropic.ts) - Anthropic Messages API
- [mistral.ts](https://github.com/matrixmapai/openabcode/blob/main/packages/ai/src/providers/mistral.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/matrixmapai/openabcode/blob/main/packages/ai/src/providers/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/matrixmapai/openabcode/blob/main/packages/ai/src/providers/openai-responses.ts) - OpenAI Responses API
- [google.ts](https://github.com/matrixmapai/openabcode/blob/main/packages/ai/src/providers/google.ts) - Google Generative AI
- [amazon-bedrock.ts](https://github.com/matrixmapai/openabcode/blob/main/packages/ai/src/providers/amazon-bedrock.ts) - AWS Bedrock
...
When a request exceeds the model's context window, openabcode can recover automatically by compacting the conversation and retrying. This recovery only kicks in if openabcode recognizes the failure as an overflow.
...
export default function (openabcode: ExtensionAPI) {
  ...
  openabcode.on("message_end", (event, ctx) => {
...
`message_end` runs before openabcode tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what openabcode checks. With this in place, openabcode will:
...
- Match a provider-specific pattern, not openabcode's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of openabcode's normal retry-with-backoff path.
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/matrixmapai/openabcode/tree/main/packages/ai/test):
...
interface ProviderModelConfig {
  ...
  /** Maps openabcode thinking levels to provider/model-specific values; null marks a level unsupported. */
