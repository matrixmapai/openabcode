...
**Note for Node.js/TypeScript users**: If you're building a Node.js application, consider using `AgentSession` directly from `@openabcode/coding-agent` instead of spawning a subprocess. See [`src/core/agent-session.ts`](../src/core/agent-session.ts) for the API. For a subprocess-based TypeScript client, see [`src/modes/rpc/rpc-client.ts`](../src/modes/rpc/rpc-client.ts).

## Starting RPC Mode

```bash
openabcode --mode rpc [options]
```
...
**Extension commands**: If the message is an extension command (e.g., `/mycommand`), it executes immediately even during streaming. Extension commands manage their own LLM interaction via `openabcode.sendMessage()`.
...
```json
{
  ...
  "data": {
    ...
    "cancelled": false,
    "truncated": true,
    "fullOutputPath": "/tmp/openabcode-bash-abc123.log"
  }
}
```
...
The current session name is available via `get_state` in the `sessionName` field. To set the initial name when starting RPC mode, pass `--name <name>` or `-n <name>` to the `openabcode --mode rpc` process.
...
```json
{
  ...
  "data": {
    "commands": [
      {"name": "session-name", "description": "Set or clear session name", "source": "extension", "path": "/home/user/.openabcode/agent/extensions/session.ts"},
      {"name": "fix-tests", "description": "Fix failing tests", "source": "prompt", "location": "project", "path": "/home/user/myproject/.openabcode/agent/prompts/fix-tests.md"},
      {"name": "skill:brave-search", "description": "Web search via Brave API", "source": "skill", "location": "user", "path": "/home/user/.openabcode/agent/skills/brave-search/SKILL.md"}
    ]
  }
}
```
...
- `source`: What kind of command:
  - `"extension"`: Registered via `openabcode.registerCommand()` in an extension
  - `"prompt"`: Loaded from a prompt template `.md` file
  - `"skill"`: Loaded from a skill directory (name is prefixed with `skill:`)
- `location`: Where it was loaded from (optional, not present for extensions):
  - `"user"`: User-level (`~/.openabcode/agent/`)
  - `"project"`: Project-level (`./.openabcode/agent/`)
...
Emitted after the full session-level run settles. At this point OpenABCode will not continue automatically through retry, compaction retry, or queued follow-up messages.
...
const agent = spawn("openabcode", ["--mode", "rpc", "--no-session"]);
