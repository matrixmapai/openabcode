> openabcode can create themes. Ask it to build one for your setup.
...
OpenABCode loads themes from:

- Built-in: `dark`, `light`
- Global: `~/.openabcode/agent/themes/*.json`
- Project: `.openabcode/themes/*.json` (only after the project is trusted)
- Packages: `themes/` directories or `openabcode.themes` entries in `package.json`
...
On first run, openabcode detects your terminal background and defaults to `dark` or `light`.
...
```bash
mkdir -p ~/.openabcode/agent/themes
vim ~/.openabcode/agent/themes/my-theme.json
```
...
```json
{
  "$schema": "https://raw.githubusercontent.com/matrixmapai/openabcode/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, openabcode reloads it automatically for immediate visual feedback.
...
OpenABCode uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, openabcode falls back to the nearest approximation.
