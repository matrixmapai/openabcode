> openabcode can create TUI components. Ask it to build one for your use case.

# TUI Components

Extensions and custom tools can render custom TUI components for interactive user interfaces. This page covers the component system and available building blocks.

**Source:** [`@openabcode/tui`](https://github.com/matrixmapai/openabcode/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@openabcode/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with `showHardwareCursor`, `setShowHardwareCursor(true)`, or `OPENABCODE_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.
...
```typescript
import { Container, type Focusable, Input } from "@openabcode/tui";
...
**In custom tools** via `openabcode.ui.custom()`:

```typescript
async execute(toolCallId, params, onUpdate, ctx, signal) {
  const handle = openabcode.ui.custom(myComponent);
  // ...
  handle.close();
}
```
...
Import from `@openabcode/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@openabcode/tui";
```
...
```typescript
import { matchesKey, Key } from "@openabcode/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@openabcode/tui";
...
```typescript
openabcode.registerCommand("pick", {
...
```typescript
import { getMarkdownTheme } from "@openabcode/coding-agent";
import { Markdown } from "@openabcode/tui";
...
Set `OPENABCODE_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
OPENABCODE_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@openabcode/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@openabcode/coding-agent";
import { DynamicBorder } from "@openabcode/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@openabcode/tui";
...
```typescript
import { BorderedLoader } from "@openabcode/coding-agent";

openabcode.registerCommand("fetch", {
  handler: async (_args, ctx) => {
    const result = await ctx.ui.custom<string | null>((tui, theme, _kb, done) => {
...
For toggling multiple settings. Use `SettingsList` from `@openabcode/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@openabcode/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@openabcode/tui";

openabcode.registerCommand("settings", {
  handler: async (_args, ctx) => {
...
Customize the inline working indicator shown while openabcode is streaming a response.
...
// Restore openabcode's default spinner
ctx.ui.setWorkingIndicator();
```
...
import { matchesKey, truncateToWidth } from "@openabcode/tui";
