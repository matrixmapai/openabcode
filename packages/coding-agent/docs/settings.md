...
OpenABCode uses JSON settings files with project settings overriding global settings.

| Location | Scope |
|----------|-------|
| `~/.openabcode/agent/settings.json` | Global (all projects) |
| `.openabcode/settings.json` | Project (current directory) |
...
| `enableAnalytics` | boolean | `false` | Opt-in analytics data sharing. Currently only asked for during the experimental first-time setup (`OPENABCODE_EXPERIMENTAL=1`) |
...
For VS Code, include `--wait` so openabcode resumes after the editor exits:
...
Keep `retry.provider.maxRetries` at `0` unless provider-level retries are explicitly needed. Setting it above `0` can make SDK/provider retries handle out-of-usage-limit errors before OpenABCode sees them, which may block the agent until the provider quota resets in some circumstances.
...
`npmCommand` is used for all npm package-manager operations, including installs, uninstalls, and dependency installs inside git packages. User-scoped npm packages install under `~/.openabcode/agent/npm/`; project-scoped npm packages install under `.openabcode/npm/`. Use argv-style entries exactly as the process should be launched. When `npmCommand` is configured, git package dependency installs use plain `install` to avoid npm-specific flags in wrappers or alternate package managers.
...
```json
{ "sessionDir": ".openabcode/sessions" }
```

When multiple sources specify a session directory, precedence is `--session-dir`, `OPENABCODE_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.openabcode/agent/settings.json` resolve relative to `~/.openabcode/agent`. Paths in `.openabcode/settings.json` resolve relative to `.openabcode`. Absolute paths and `~` are supported.
...
```json
{
  "packages": ["openabcode-skills", "@org/my-extension"]
}
```
...
```json
{
  ...
  "packages": ["openabcode-skills"]
}
```

## Project Overrides

Project settings (`.openabcode/settings.json`) override global settings. Nested objects are merged:

```json
// ~/.openabcode/agent/settings.json (global)
{
  "theme": "dark",
  "compaction": { "enabled": true, "reserveTokens": 16384 }
}

// .openabcode/settings.json (project)
{
