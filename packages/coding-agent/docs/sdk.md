> openabcode can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to openabcode's agent capabilities. Use it to embed openabcode in other applications, build custom interfaces, or integrate with automated workflows.
...
```bash
npm install @openabcode/coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@openabcode/coding-agent";
...
**Behavior:**
- **Extension commands** (e.g., `/mycommand`): Execute immediately, even during streaming. They manage their own LLM interaction via `openabcode.sendMessage()`.
...
The `Agent` class (from `@openabcode/agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.openabcode/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.openabcode/extensions/`)
- Project skills:
  - `.openabcode/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.openabcode/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.openabcode/agent/skills/`)
  - `~/.agents/skills/`
...
```typescript
import { getModel } from "@openabcode/ai";
import { AuthStorage, ModelRegistry } from "@openabcode/coding-agent";
...
// Default: uses ~/.openabcode/agent/auth.json and ~/.openabcode/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@openabcode/coding-agent";
...
The `edit` tool returns `details.diff` for OpenABCode's TUI display and `details.patch` as a standard unified patch for SDK consumers.

```typescript
import { createAgentSession } from "@openabcode/coding-agent";
...
import { createAgentSession, defineTool } from "@openabcode/coding-agent";
...
Use `defineTool()` for standalone definitions and arrays like `customTools: [myTool]`. Inline `openabcode.registerTool({ ... })` already infers parameter types correctly.

Custom tools passed via `customTools` are combined with extension-registered tools. Extensions loaded by the ResourceLoader can also register tools via `openabcode.registerTool()`.
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.openabcode/agent/extensions/`, `.openabcode/extensions/`, and settings.json extension sources.
...
```typescript
import type { InlineExtension } from "@openabcode/coding-agent";
...
**Event Bus:** Extensions can communicate via `openabcode.events`. Pass a shared `eventBus` to `DefaultResourceLoader` if you need to emit or listen from outside:

```typescript
import { createEventBus, DefaultResourceLoader } from "@openabcode/coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@openabcode/coding-agent";
...
1. Global: `~/.openabcode/agent/settings.json`
2. Project: `<cwd>/.openabcode/settings.json`
...
```bash
openabcode --mode rpc --no-session
```
