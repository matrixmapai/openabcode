import { Markdown, type MarkdownTheme } from "@openabcode/tui";
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		...
		case "update":
			return `${APP_NAME} update [source|self|openabcode] [--self|--extensions|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
...
Update openabcode and installed packages.

Options:
  --self                  Update openabcode only (default when no target is given)
  --extensions            Update installed packages only
  --all                   Update openabcode and installed packages
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reinstall openabcode even if the current version is latest

Short forms:
  ${APP_NAME} update                Update openabcode only
  ${APP_NAME} update --all          Update openabcode and all extensions
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update openabcode     Update openabcode only (self works as alias)
`);
			return;
...
function parsePackageCommand(args: string[]): PackageCommandOptions | undefined {
	...
	if (command === "update") {
		...
		if (extensionFlagSource) {
			...
			const sourceIsSelf = source === "self" || source === "pi" || source === "openabcode";
			if (sourceIsSelf) {
...
function printSelfUpdateUnavailable(
	...
	if (entrypoint) {
		console.error("");
		console.error(`Location of openabcode executable: ${entrypoint}`);
	}
}
