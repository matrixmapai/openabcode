import { type AgentMessage, uuidv7 } from "@openabcode/agent-core";
 ...
 * Encodes cwd into a safe directory name under ~/.openabcode/agent/sessions/.
 */
...
export class SessionManager {
	...
	setSessionFile(sessionFile: string): void {
		...
		if (existsSync(this.sessionFile)) {
			...
			// non-empty but did not parse as a openabcode session, fail without modifying it.
			if (this.fileEntries.length === 0) {
				const explicitPath = this.sessionFile;
				if (statSync(explicitPath).size > 0) {
					throw new Error(`Session file is not a valid openabcode session: ${explicitPath}`);
				}
				this.newSession();
	 ...
	 * @param sessionDir Optional session directory. If omitted, uses default (~/.openabcode/agent/sessions/<encoded-cwd>/).
	 */
