...
function isOfflineModeEnabled(): boolean {
	const value = process.env.OPENABCODE_OFFLINE;
...
export class DefaultPackageManager implements PackageManager {
	...
	private getNpmInstallArgs(specs: string[], installRoot: string): string[] {
		...
		// Extension packages run inside openabcode and resolve openabcode APIs through loader aliases/virtual modules.
		// Disable peer dependency resolution for managed installs (npm's --legacy-peer-deps, and
		// equivalent bun/pnpm settings) so package managers do not install or solve host-provided
		// @openabcode/* peers. Stale auto-installed openabcode peers can otherwise block updates.
	...
	private ensureNpmProject(installRoot: string): void {
		...
		if (!existsSync(packageJsonPath)) {
			const pkgJson = { name: "openabcode-extensions", private: true };
	...
	private addAutoDiscoveredResources(
		...
		if (projectTrusted) {
			// Project extensions from .openabcode/
			addResources(
				"extensions",
			...
			// Project skills from .openabcode/
			addResources(
				"skills",
		...
		// User extensions from ~/.openabcode/agent/
		addResources(
			"extensions",
		...
		// User skills from ~/.openabcode/agent/
		addResources(
			"skills",
