...
const originalPiPackageDir = process.env.OPENABCODE_PACKAGE_DIR;
...
afterEach(() => {
	...
	if (originalPiPackageDir === undefined) {
		delete process.env.OPENABCODE_PACKAGE_DIR;
	} else {
		process.env.OPENABCODE_PACKAGE_DIR = originalPiPackageDir;
	}
...
function createNpmPrefixInstall(template = "pi-prefix-"): { prefix: string; packageDir: string } {
	...
	tempDir = prefix;
	process.env.OPENABCODE_PACKAGE_DIR = packageDir;
...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@openabcode/coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @openabcode/coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@openabcode/coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@openabcode/coding-agent")).toBe(
			"Update @openabcode/coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@openabcode/coding-agent");
		...
		expect(command).toEqual({
			...
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @openabcode/coding-agent`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@openabcode/coding-agent", undefined, {
			packageName: "@openabcode/coding-agent",
			installSpec: "@openabcode/coding-agent@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@openabcode/coding-agent@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @openabcode/coding-agent@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@openabcode/coding-agent", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @openabcode/coding-agent`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@openabcode/coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @openabcode/coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@openabcode/coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @openabcode/coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@openabcode/coding-agent";
		...
		tempDir = temp;
		process.env.PATH = `${binDir}${delimiter}${originalPath ?? ""}`;
		process.env.OPENABCODE_PACKAGE_DIR = storePackageDir;
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@openabcode/coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@openabcode/coding-agent")).toContain(
