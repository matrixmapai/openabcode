...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".openabcode", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".openabcode", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".openabcode", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".openabcode"), { recursive: true });
			writeFileSync(join(cwd, ".openabcode", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".openabcode"), { recursive: true, force: true });
