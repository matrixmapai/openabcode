...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.OPENABCODE_OFFLINE;
		delete process.env.OPENABCODE_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.OPENABCODE_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .openabcode skill dirs", async () => {
...
Content`,
		...
		it("should resolve project paths relative to .openabcode", async () => {
			const extDir = join(tempDir, ".openabcode", "extensions");
			mkdirSync(extDir, { recursive: true });
...
Content`,
				...
				mkdirSync(join(tempDir, ".openabcode"), { recursive: true });
				...
				symlinkSync(sharedExtensionsDir, join(tempDir, ".openabcode", "extensions"), "dir");
				symlinkSync(sharedSkillsDir, join(tempDir, ".openabcode", "skills"), "dir");
				symlinkSync(sharedPromptsDir, join(tempDir, ".openabcode", "prompts"), "dir");
				symlinkSync(sharedThemesDir, join(tempDir, ".openabcode", "themes"), "dir");
		...
		it("should auto-discover project prompts with overrides", async () => {
			const promptsDir = join(tempDir, ".openabcode", "prompts");
			mkdirSync(promptsDir, { recursive: true });
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .openabcode/agent skills", async () => {
		...
		it("should use the project .openabcode dir as baseDir for project .openabcode skills", async () => {
			const projectBaseDir = join(tempDir, ".openabcode");
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should keep ~/.agents/skills user-scoped when cwd is under home in a non-git directory", async () => {
			...
			try {
				...
				const localAgentDir = join(tempDir, ".openabcode", "agent");
		...
		it("should dedupe user skill entries when ~/.openabcode/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("ignore files", () => {
		...
		it("should not apply parent .gitignore to .openabcode auto-discovery", async () => {
			writeFileSync(join(tempDir, ".gitignore"), ".openabcode\n");

			const skillDir = join(tempDir, ".openabcode", "skills", "auto-skill");
			mkdirSync(skillDir, { recursive: true });
	...
	describe("resolveExtensionSources", () => {
		...
		it("should handle directories with openabcode manifest", async () => {
		...
		it("should keep openabcode manifest entries with leading tilde package-relative", async () => {
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.openabcode\\npm";
	...
	describe("npmCommand", () => {
		...
		it("should update git package dependencies with --omit=dev", async () => {
			const source = "git:github.com/user/repo";
			const targetDir = join(tempDir, ".openabcode", "git", "github.com", "user", "repo");
			mkdirSync(targetDir, { recursive: true });
			writeFileSync(join(targetDir, "package.json"), JSON.stringify({ name: "repo", version: "1.0.0" }));
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .openabcode settings base", () => {
			...
			const rel = relative(join(tempDir, ".openabcode"), projectPkgDir);
			const expected = rel.startsWith(".") ? rel : `./${rel}`;
			expect(settings.packages?.[0]).toBe(expected);
		});
	...
	describe("pattern filtering in openabcode manifest", () => {
	...
	describe("pattern filtering in package filters", () => {
		...
		it("should resolve autoload-disabled package entries as positive-only without a global package", async () => {
			...
			settingsManager.setProjectPackages([
				{
					source: relative(join(tempDir, ".openabcode"), pkgDir),
					autoload: false,
			...
			expect(result.skills.filter((resource) => resource.path.startsWith(pkgDir))).toEqual([]);
		});
	});
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
	...
	describe("offline mode and network timeouts", () => {
		it("should update npm range packages using the configured spec", async () => {
			const installedPath = join(tempDir, ".openabcode", "npm", "node_modules", "example");
			...
			expect(runCommandSpy).toHaveBeenCalledWith(
				"npm",
				["install", "example@^1.0.0", "--prefix", join(tempDir, ".openabcode", "npm"), "--legacy-peer-deps"],
				undefined,
			);
		});
		...
		it("should batch npm updates per scope and run git updates in parallel while skipping pinned npm and current packages", async () => {
			...
			const projectOldPath = join(tempDir, ".openabcode", "npm", "node_modules", "project-old");
			const projectCurrentPath = join(tempDir, ".openabcode", "npm", "node_modules", "project-current");
		...
		it("should skip installing missing package sources when offline", async () => {
			process.env.OPENABCODE_OFFLINE = "1";
		...
		it("should skip pinned packages when checking for updates", async () => {
			const installedNpmPath = join(tempDir, ".openabcode", "npm", "node_modules", "example");
