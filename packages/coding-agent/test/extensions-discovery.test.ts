...
describe("extensions discovery", () => {
	...
	it("discovers subdirectory with package.json openabcode field", async () => {
	...
	it("keeps package.json openabcode extension entries with leading tilde package-relative", async () => {
	...
	it("package.json with openabcode field takes precedence over index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
	...
	it("ignores package.json without openabcode field, falls back to index.ts", async () => {
		const subdir = path.join(extensionsDir, "my-package");
		fs.mkdirSync(subdir);
		fs.writeFileSync(path.join(subdir, "index.ts"), extensionCode);
		fs.writeFileSync(
