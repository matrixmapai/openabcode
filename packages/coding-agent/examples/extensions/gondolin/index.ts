 ...
 *
 * Runs openabcode's built-in tools inside a local Gondolin micro-VM. The host working
 ...
 * Usage:
 *   cd /path/to/project
 *   openabcode -e /path/to/openabcode/packages/coding-agent/examples/extensions/gondolin
 *
 * Requirements:
...
import type { ExtensionAPI, ExtensionContext } from "@openabcode/coding-agent";
import {
	type BashOperations,
	createBashTool,
	createEditTool,
...
export default function (api: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const created = await VM.create({
			sessionLabel: `openabcode ${path.basename(localCwd)}`,
			vfs: {
				mounts: {
