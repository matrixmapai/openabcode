...
```bash
npm install @openabcode/ai
```

TypeBox exports are re-exported from `@openabcode/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@openabcode/ai';
import { builtinModels } from '@openabcode/ai/providers/all';
...
```typescript
import { anthropicProvider } from '@openabcode/ai/providers/anthropic';
import { openaiProvider } from '@openabcode/ai/providers/openai';
import { openrouterProvider } from '@openabcode/ai/providers/openrouter';
import { amazonBedrockProvider } from '@openabcode/ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@openabcode/ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@openabcode/ai/providers/all';
...
Stored credentials (API keys entered interactively, OAuth tokens) live in a `CredentialStore` — one type-tagged credential per provider. openabcode-ai ships an in-memory default; apps inject persistent storage:

```typescript
import { createModels, type CredentialStore } from '@openabcode/ai';
...
API-key credentials use the same discriminator as openabcode's `auth.json` and can carry provider-scoped env/config values:
...
```typescript
import { Type, type Tool, StringEnum } from '@openabcode/ai';
...
```typescript
import { validateToolCall, type Tool } from '@openabcode/ai';
...
```typescript
import { builtinImagesModels } from '@openabcode/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@openabcode/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@openabcode/ai';
import { openAICompletionsApi } from '@openabcode/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@openabcode/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@openabcode/ai/api/openai-responses.lazy';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are openabcode thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`). Missing standard levels through `high` use provider defaults; `xhigh` and `max` are opt-in and require a non-null map entry. String values are sent to the provider, `null` marks a level unsupported, and maps may skip levels.
...
```typescript
import { stream } from '@openabcode/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@openabcode/ai/api/<api-id>`.
...
interface OpenAICompletionsCompat {
  ...
  chatTemplateKwargs?: Record<string, string | number | boolean | null | { '$var': 'thinking.enabled' | 'thinking.effort'; omitWhenOff?: boolean }>; // chat_template_kwargs values; use $var for openabcode-controlled thinking values
...
```typescript
import { createModels, type Context } from '@openabcode/ai';
import { anthropicProvider } from '@openabcode/ai/providers/anthropic';
import { openaiProvider } from '@openabcode/ai/providers/openai';
import { googleProvider } from '@openabcode/ai/providers/google';
...
```typescript
import { createModels } from '@openabcode/ai';
import { anthropicProvider } from '@openabcode/ai/providers/anthropic';
...
- `@openabcode/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@openabcode/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@openabcode/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@openabcode/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@openabcode/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@openabcode/ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@openabcode/ai/bedrock-provider';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `OPENABCODE_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @openabcode/ai login              # interactive provider selection
npx @openabcode/ai login anthropic    # login to specific provider
npx @openabcode/ai list               # list available providers
```
...
The legacy flow functions remain available via the `@openabcode/ai/oauth` entry point (`loginAnthropic`, `loginOpenAICodex`, `loginGitHubCopilot`, `refreshOAuthToken`, `getOAuthApiKey`); credential storage is the caller's responsibility there. New code should prefer the provider-owned `OAuthAuth` shown above — it composes with the credential store and gets locked auto-refresh for free.
...
```typescript
// Before
import { getModel, complete } from '@openabcode/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@openabcode/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@openabcode/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@openabcode/ai`.
