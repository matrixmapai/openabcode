...
// OpenAI Responses rejects max_output_tokens below 16: https://github.com/matrixmapai/openabcode/issues/6265
 ...
 * Defaults to "short" and uses OPENABCODE_CACHE_RETENTION for backward compatibility.
 */
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention {
	if (cacheRetention) {
		return cacheRetention;
	}
	if (getProviderEnvValue("OPENABCODE_CACHE_RETENTION", env) === "long") {
		return "long";
	}
	return "short";
}
...
function buildParams(model: Model<"openai-responses">, context: Context, options?: OpenAIResponsesOptions) {
	...
	if (model.reasoning && (model.compat?.supportsReasoningEffort ?? true)) {
